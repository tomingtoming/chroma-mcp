...
async def chroma_create_collection(
    ...
    client.create_collection(name=collection_name, metadata=metadata if metadata else None)
...
async def chroma_modify_collection(
    collection_name: str, new_name: Optional[str] = None, new_metadata: Optional[Dict] = None
) -> str:
...
async def chroma_delete_documents(
    ...
    if ids is None and where is None and where_document is None:
        raise ValueError(
            "No deletion criteria provided. Please specify 'ids', 'where', or 'where_document'."
        )
    ...
    if ids is not None and (where is not None or where_document is not None):
        raise ValueError(
            "Cannot provide both 'ids' and filtering conditions ('where' or 'where_document'). Please use either 'ids' or filters."
        )
...
def find_similar_sessions(
    thoughts_collection, summary_collection, thought, similarity_threshold=0.75, max_results=3
):
    ...
    for i in range(len(similar_thoughts["ids"][0])):
        ...
        if distance < similarity_threshold:
            ...
            summary_results = summary_collection.get(
                ids=[session_id], include=["documents", "metadatas"]
            )

            if summary_results["documents"]:
                summary = summary_results["documents"][0]
                summary_metadata = (
                    summary_results["metadatas"][0] if summary_results["metadatas"] else {}
                )
                ...
                context_results.append(
                    {
                        ...
                        "keyThoughts": json.loads(summary_metadata.get("keyThoughts", "[]"))
                        if summary_metadata
                        else [],
                    }
                )
...
async def chroma_sequential_thinking(
    ...
    if persist:
        client = get_chroma_client()
        thoughts_collection, branches_collection, summary_collection = init_thinking_collections(
            client
        )
        ...
        if thoughtNumber == 1:
            similar_sessions = find_similar_sessions(
                thoughts_collection, summary_collection, thought
            )
        else:
            similar_sessions = []
        ...
        # Store in Chroma
        thoughts_collection.add(documents=[thought], metadatas=[metadata], ids=[thought_id])
        ...
        if sessionSummary:
            ...
            try:
                summary_result = summary_collection.get(
                    ids=[sessionId], include=["documents", "metadatas"]
                )
            ...
            if existing_summary:
                ...
                summary_collection.update(
                    ids=[sessionId], documents=[sessionSummary], metadatas=[summary_metadata]
                )
            else:
                summary_collection.add(
                    documents=[sessionSummary], metadatas=[summary_metadata], ids=[sessionId]
                )
...
async def chroma_get_similar_sessions(
    ...
    results = thoughts_collection.query(
        query_texts=[text], n_results=5, include=["documents", "metadatas"]
    )
...
async def chroma_get_thought_history(
    ...
    results = thoughts_collection.get(where={"sid": sessionId}, include=["documents", "metadatas"])

    # Sort by thought number
    thoughts = []
    for i, doc in enumerate(results["documents"]):
        metadata = results["metadatas"][i]
        thoughts.append({"thought": doc, "metadata": metadata})

    # Sort thoughts by number
    thoughts.sort(key=lambda x: x["metadata"]["tn"])

    return {"sessionId": sessionId, "thoughts": thoughts, "totalThoughts": len(thoughts)}
...
async def chroma_get_thought_branches(sessionId: str) -> Dict:
    ...
    results = branches_collection.get(where={"sid": sessionId}, include=["metadatas"])

    branches = []
    for metadata in results["metadatas"]:
        branches.append({"branchId": metadata["bid"], "parentThought": metadata["pt"]})

    return {"sessionId": sessionId, "branches": branches, "totalBranches": len(branches)}


@mcp.tool()
async def chroma_continue_thought_chain(sessionId: str, branchId: Optional[str] = None) -> Dict:
    ...
    results = thoughts_collection.get(where=where_clause, include=["documents", "metadatas"])

    if not results["documents"]:
        return {
            "error": f"No thoughts found for session {sessionId}"
            + (f" and branch {branchId}" if branchId else ""),
            "status": "failed",
        }
    ...
    for i, metadata in enumerate(results["metadatas"]):
        if metadata["tn"] > max_thought_num:
            ...
            latest_thought = {"thought": results["documents"][i], "metadata": metadata}
