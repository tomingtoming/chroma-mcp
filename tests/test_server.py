...
async def test_create_and_delete_collection():
    ...
    delete_result = await mcp.call_tool(
        "chroma_delete_collection", {"collection_name": collection_name}
    )
...
def test_cmd_args_override_env_vars(mock_env_vars):
    ...
    args = parser.parse_args(
        ["--client-type", "persistent", "--data-dir", "/test/dir", "--ssl", "true"]
    )
...
def test_persistent_client_creation(mock_persistent_client, mock_env_vars):
    ...
    # Set up command line args
    sys.argv = ["chroma-mcp", "--client-type", "persistent", "--data-dir", "/test/data/dir"]
...
async def test_update_documents_success():
    ...
    updated_metadatas = [
        initial_metadatas[0],
        {"source": "updated"},
    ]  # Update only second doc metadata
    ...
    try:
        ...
        # 4. Verify updates
        get_result_raw = await mcp.call_tool(
...
async def test_update_documents_invalid_args():
    ...
    try:
        ...
        await mcp.call_tool(
            "chroma_add_documents",
            {"collection_name": collection_name, "documents": ["Test doc"], "ids": ["doc1"]},
        )
        ...
        with pytest.raises(ToolError, match="The 'ids' list cannot be empty."):
            await mcp.call_tool(
                "chroma_update_documents",
                {"collection_name": collection_name, "ids": [], "documents": ["New content"]},
            )
...
async def test_update_documents_id_not_found():
    ...
    try:
        ...
        await mcp.call_tool(
            "chroma_add_documents",
            {"collection_name": collection_name, "documents": ["Test doc"], "ids": ["existing_id"]},
        )
        ...
        get_result_raw = await mcp.call_tool(
            "chroma_get_documents",
            {"collection_name": collection_name, "ids": ["existing_id"], "include": ["documents"]},
        )
...
async def test_delete_documents_by_ids(setup_delete_test_collection):
    ...
    delete_result = await mcp.call_tool(
        "chroma_delete_documents", {"collection_name": collection_name, "ids": ids_to_delete}
    )
    ...
    assert (
        f"Successfully processed delete request for collection '{collection_name}'"
        in delete_result[0].text
    )
...
async def test_delete_documents_by_where(setup_delete_test_collection):
    ...
    delete_result = await mcp.call_tool(
        "chroma_delete_documents", {"collection_name": collection_name, "where": where_filter}
    )
    assert isinstance(delete_result, list)
    assert len(delete_result) >= 1
    assert hasattr(delete_result[0], "text")
...
async def test_delete_documents_no_criteria_error(setup_delete_test_collection):
    ...
    with pytest.raises(ToolError, match="No deletion criteria provided"):
        await mcp.call_tool("chroma_delete_documents", {"collection_name": collection_name})
...
async def test_delete_documents_both_ids_and_filter_error(setup_delete_test_collection):
    ...
    with pytest.raises(ToolError, match="Cannot provide both 'ids' and filtering conditions"):
        await mcp.call_tool(
            "chroma_delete_documents",
            {"collection_name": collection_name, "ids": ["id1"], "where": {"type": "a"}},
        )
...
async def test_delete_documents_nonexistent_collection():
    ...
    with pytest.raises(ToolError):  # Catch ToolError wrapping the underlying exception
        await mcp.call_tool(
            "chroma_delete_documents",
            {"collection_name": collection_name, "ids": ["id_does_not_matter"]},
        )
